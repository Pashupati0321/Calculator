...
const Controls = ({ firstNumber, secondNumber, setTotalResult, setInputLabel, setFirstNumber, setSecondNumber }) => {
  ...
  const remainder = () => {
    if (validateInputs()) {
      setInputLabel("***Please enter the blank field(s)");
      console.log("Result Status: Remainder clicked : Please enter the blank input field(s)");
    } else {
      const result = parseInt(firstNumber) % parseInt(secondNumber);
      setTotalResult(result.toString());
  ...
  return (
    <>
      ...
      <Button sx={{ margin: 2, fontWeight: 'bold' }} variant="contained" color="warning" onClick={remainder}>%</Button>
      <Button sx={{ margin: 2, fontWeight: 'bold' }} variant="contained" color="primary" onClick={factorial}>Factorial</Button>
